...
{
    LV_COLOR_MAKE(0xFF, 0x33, 0x33),    // Fire Engine Red
    LV_COLOR_MAKE(0x33, 0xD5, 0xE5),    // Cool Ocean Blue
    LV_COLOR_MAKE(0xFF, 0xFF, 0xFF),    // Black & White
    LV_COLOR_MAKE(0x3B, 0xB0, 0x3B)     // Moss Green
};
...
static void add_item_to_table(lv_obj_t* table, uint16_t row, const char* alarm_text, const TIME_INFO* trigger_time, uint32_t trigger_days)
{
    ...
    lv_table_set_cell_align(table, row, 0, LV_LABEL_ALIGN_CENTER);
    lv_table_set_cell_align(table, row, 1, LV_LABEL_ALIGN_CENTER);
    lv_table_set_cell_align(table, row, 2, LV_LABEL_ALIGN_CENTER);
...
static void save_alarm_tab_cb(lv_obj_t* save_btn, lv_event_t event)
{
    if (event == LV_EVENT_CLICKED)
    {
...
static void alarm_tbl_event_cb(lv_obj_t* table_obj, lv_event_t event)
{
    switch(event)
    {
        case LV_EVENT_PRESSED:
        {
            ...
            if (gui_info != NULL && lv_table_get_pressed_cell(table_obj, &row, &col) == LV_RES_OK)
            {
                ...
                if (row > 0 && row <= alarm_count)
                {
                    ...
                    lv_textarea_set_cursor_hidden(gui_info->new_alarm_dlg.alarm_text, true);
...
static void create_new_alarm_tab(GUI_MGR_INFO* gui_info, lv_obj_t* parent)
{
    ...
    lv_coord_t left = LEFT_MARGIN;
    lv_coord_t top = TOP_MARGIN;
    struct tm* curr_time = get_time_value();
    ...
    lv_obj_align(save_btn, NULL, LV_ALIGN_IN_TOP_RIGHT, -20, TOP_MARGIN);
    ...
    lv_obj_set_width(gui_info->new_alarm_dlg.delete_btn, 200);
    ...
    lv_obj_set_pos(gui_info->new_alarm_dlg.alarm_text, 290, TOP_MARGIN);

    // Create a container
    lv_obj_t* cont = lv_cont_create(parent, NULL);
    lv_obj_set_pos(cont, LEFT_MARGIN, top);
    lv_obj_set_size(cont, 470, 110);
...
static void create_option_tab(GUI_MGR_INFO* gui_info, lv_obj_t* parent)
{
    ...
    const uint16_t label_left = LEFT_MARGIN+440;
    const uint16_t ctrl_pos_left = LEFT_MARGIN+630;

    lv_obj_t* zip_label = lv_label_create(parent, NULL);
    lv_label_set_text(zip_label, "Zip Code");
    lv_obj_set_pos(zip_label, label_left, TOP_MARGIN+5);
    ...
    lv_obj_set_pos(hour_label, label_left, TOP_MARGIN+50);
    ...
    lv_obj_set_pos(temp_unit_label, label_left, TOP_MARGIN+100);
    ...
    create_time_roller(parent, label_left, TOP_MARGIN+150, &gui_info->new_alarm_dlg.start_shade, &shade_time);
    shade_time.tm_hour = 7;
    shade_time.tm_min = 00;
    create_time_roller(parent, label_left, TOP_MARGIN+200, &gui_info->new_alarm_dlg.end_shade, &shade_time);*/
    ...
    lv_obj_set_pos(num_keybrd, LEFT_MARGIN-10, TOP_MARGIN+150);
    ...
    lv_obj_set_pos(version_label, LEFT_MARGIN, TOP_MARGIN+360);
...
void gui_mgr_show_alarm_dlg(GUI_MGR_HANDLE handle, SCHEDULER_HANDLE sched_handle)
{
    if (handle != NULL && sched_handle != NULL)
    {
        ...
        handle->new_alarm_dlg.tab_view = lv_tabview_create(alarm_opt_win, NULL);
        lv_obj_t* alarm_tab = lv_tabview_add_tab(handle->new_alarm_dlg.tab_view, "Alarms");
        lv_obj_t* new_alarm_tab = lv_tabview_add_tab(handle->new_alarm_dlg.tab_view, "New Alarm");
        lv_obj_t* option_tab = lv_tabview_add_tab(handle->new_alarm_dlg.tab_view, "Options");

        // Setup alarm tab
        create_alarm_tab(handle, alarm_tab);

        // Setup New Alarm tab
        create_new_alarm_tab(handle, new_alarm_tab);

        // Setup Option tab
        create_option_tab(handle, option_tab);
    }
}